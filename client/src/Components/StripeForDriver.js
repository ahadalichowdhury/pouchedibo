...
function StripeForDriver() {
  ...
  useEffect(() => {
    const loadStripe = async () => {
      ...
      const stripeObject = await stripeModule.loadStripe(
        "pk_test_51OEnDEE7CJNVLFNH7NY5UAy4dvnMbmcQufcniFdAJLq66B1Qy9dZhIOzqsNooORfpUhOGKQEiEhde9fKG0sIrItV00hQp8a0kZ"
      );
  ...
  const handlePaymentClick = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/v1/make-instructor",
        {
          email: email,
        }
      );
  ...
  return (
    <div className="become-driver">
      ...
      <p>Do you want to become a driver. Please click here </p>

      <button
        className="btn w-10 float-center btn-primary "
